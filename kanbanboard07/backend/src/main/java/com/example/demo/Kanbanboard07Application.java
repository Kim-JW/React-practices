...
public class Kanbanboard07Application {
	...
	public ApplicationRunner scriptRunner() {
		return new ApplicationRunner() {
			...
			@Autowired
			private SqlSessionFactory sqlSessonFactory;
			
			@Override
			public void run(ApplicationArguments args) throws Exception {
				ScriptRunner scriptRunner = new ScriptRunner(sqlSessonFactory.getConfiguration().getEnvironment().getDataSource().getConnection());
